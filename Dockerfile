...
# Download the vllm v0.6.2 wheel first, then install locally
RUN wget https://github.com/vllm-project/vllm/releases/download/v0.6.2/vllm-0.6.2+cu118-cp310-cp310-manylinux1_x86_64.whl -O /tmp/vllm.whl
RUN pip3 install --no-cache-dir /tmp/vllm.whl -vvv
RUN rm /tmp/vllm.whl
# Install packages from requirements.txt (runpod, orpheus-tts)
# NOTE: orpheus-tts might pull a newer vllm dependency, but pip should use the already installed v0.6.2
